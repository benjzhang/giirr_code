...
%             Gsq = sqrtm(inv(G)+eye(d)) ;
            wtemp(:,jj+1) = wtemp(:,jj) + 0.5 * h * gradlogpos_blr_irriem(wtemp(:,jj),alpha,X,ttrain,n,G,J) + sqrt(h) * (Gsq * randn(d,1) );
        end
        w5(:,:,ii) = wtemp;
    ...
    w5first = sum(w5,1) ;
    w5sec = sum(w5.^2,1);
    name = sprintf('irriem20%i.mat',kk);
    save(name,'w5first','w5sec');
end
