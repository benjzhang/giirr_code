...
parfor mm = 1:M
    ...
    Y(:,1) = initcond;
    for kk = 1:K-1
        Y(:,kk+1) = Y(:,kk) + h/2 * gradfuncsg(Y(:,kk), X) + sqrt(h) * randn(2,1) ;
    end
    Yall(:,:,mm) = Y ;
end
...
parfor mm = 1:M
    ...
    for kk = 1:K-1
        ...
        gradeval = gradfuncsg(Yirrm(:,kk),X) ;
...
name = sprintf('runmoreburn01%i.mat',ll); 
